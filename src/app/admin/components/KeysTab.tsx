        ...
        >
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
              {data.providers.find(p => p.id === selectedProvider)?.isCustom && (
                <>
                  <button
                    onClick={() => {
                      const p = data.providers.find(p => p.id === selectedProvider);
                      if (p) {
                        setEditingCustomProvider({
                          id: p.id,
                          name: p.name,
                          baseUrl: p.baseUrl,
                          headerFormat: p.headerFormat || 'openai',
